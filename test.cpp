...
bool is_log_concave(Poly& P){
    for(ll n=1;n<P.len()-1;n++){
        if(P[n].dat*P[n].dat<P[n-1].dat*P[n+1].dat){
            return false;
        }
    }
    return true;
}
bool is_unimodal(Poly& P){
    ll status=0;
    for(ll n=1;n<P.len();n++){
        if(status==0){
            if(P[n-1].dat>P[n].dat) status=1;
        }else if(status==1){
            if(P[n-1].dat<P[n].dat) return false;
        }
    } return true;
}
